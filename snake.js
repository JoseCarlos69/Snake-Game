class createSnake {
    ...
    setVel(theKeyCode){
        ...
        if(!this.gameBack.lose){
            ...
            if((theKeyCode == 37 || theKeyCode == 65) && this.permision){//esquerda
                setXY(-this.vel, 0);
                this.permision = !this.permision;
            }else if((theKeyCode == 38 || theKeyCode == 87) && !this.permision){//cima
                setXY(0, -this.vel);
                this.permision = !this.permision;
            }else if((theKeyCode == 39 || theKeyCode == 68) && this.permision){//direita
                setXY(this.vel, 0);
                this.permision = !this.permision;
            }else if((theKeyCode == 40 || theKeyCode == 83) && !this.permision){//baixo
                setXY(0, this.vel);
                this.permision = !this.permision;
            }else{
                //none
...
function createButton(app){
    ...
    divSlide.setAttribute('class', 'div-slide');
    ...
    divSlide.appendChild(left);

    const restart = document.createElement('button');
    restart.setAttribute('type', 'button');
    restart.setAttribute('id', 'restart');
    restart.textContent = 'RESTART';
    divSlide.appendChild(restart);

    const right = document.createElement('button');
    right.setAttribute('type', 'button');
    right.setAttribute('id', 'right');
    right.textContent = 'RIGHT >';
    divSlide.appendChild(right);
...
window.onload = () => {
    ...
    const app = document.getElementById9('root');
